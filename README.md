...
#### log_conversation
```
Saves a conversation turn (user message + Claude response) in the format
recognised by `idb list` and `idb timeline`.
Args: user_text (required), assistant_text (required),
      session_id (optional, auto-generated), source (optional, default: "claude-desktop")
